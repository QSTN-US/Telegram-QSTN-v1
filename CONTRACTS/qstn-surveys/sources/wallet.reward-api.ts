...
(async () => {
    ...
    let survey = client.open(await Wallet.fromInit(publicKey, surveyId, owner, BigInt(20)));
    const sender = contract.sender(keyPair.secretKey);

    //let new_init = await Wallet.init(publicKey, surveyId + BigInt(1), new_owner);
    ...
    //const to = '0QA1VLEjE8suBqr0QoIfLuFxKwB496ANsiNw7Yf-k8FQkupV'
        ...
        let non_participant = Address.parse('EQBqlsh4xzPIFJb50a1aU9Y_x9ASSz3GHtbnFePhSmEuYaSi');
        
        //let new_participant = Address.parse('EQA1VLEjE8suBqr0QoIfLuFxKwB496ANsiNw7Yf-k8FQkgwa');
        let new_participant = Address.parse('0QC0hTRLTyYxURj7dggE-HYHcQaeFqbN2KQoJKTJwqgriHCS');
        //let new_participant = Address.parse('EQAkOWOwlAgX_boTMus9uZDXaNggGmKnmbjYbV3gujuohHkm');
        // Send comment message
        //await survey.send(sender, { value: toNano('0.1') }, 'notify');
        
        await survey.send(sender, { value: toNano('0.03') }, {
            $$type: 'Reward',
        ...
        //await vault.send(sender, { value: toNano('0.1') }, "withdraw safe");
         ...
         console.log('received reward => ', await survey.getIsRewardReceived(new_participant));
         console.log('non_participant => ',await survey.getIsParticipant(non_participant));
         console.log('is participant => ',await survey.getIsParticipant(new_participant));
        console.log('All Participants => ',await survey.getAllParticipants());
