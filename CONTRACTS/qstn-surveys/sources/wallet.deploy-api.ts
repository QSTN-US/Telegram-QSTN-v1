...
(async () => {
    ...
    let init = await Wallet.init(publicKey, walletId, owner, BigInt(20));
    ...
    let deployAmount = toNano('0.39');

    // send a message on new address contract to deploy it
    let seqno: number = await contract.getSeqno();
    console.log('🛠️Preparing new outgoing massage from deployment survey contract. Seqno = ', seqno);
    console.log('Current deployment survey balance = ', fromNano(balance).toString(), '💎TON');
