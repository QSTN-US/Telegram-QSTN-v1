...
(async () => {
    ...
    //const to = '0QA1VLEjE8suBqr0QoIfLuFxKwB496ANsiNw7Yf-k8FQkupV'
        ...
        let new_participant1 = Address.parse('0QA1VLEjE8suBqr0QoIfLuFxKwB496ANsiNw7Yf-k8FQkupV');
        let new_participant2 = Address.parse('0QC0hTRLTyYxURj7dggE-HYHcQaeFqbN2KQoJKTJwqgriHCS');
        let new_participant3 = Address.parse('EQAkOWOwlAgX_boTMus9uZDXaNggGmKnmbjYbV3gujuohHkm');
        // Send comment message
        //await survey.send(sender, { value: toNano('0.1') }, 'notify');
        
        await survey.send(sender, { value: toNano('0.01') }, {
            $$type: 'AddParticipant',
            user: new_participant1,
            uservault: new_participant1,
            resultsid: BigInt(1396)
        });

        /*await survey.send(sender, { value: toNano('0.01') }, {
            $$type: 'AddParticipant',
            user: new_participant2,
            uservault: new_participant2,
            resultsid: BigInt(1403)
        });*/

        /*await survey.send(sender, { value: toNano('0.01') }, {
            $$type: 'AddParticipant',
            user: new_participant3,
            uservault: new_participant3,
            resultsid: BigInt(1419)
        });*/

        //await vault.send(sender, { value: toNano('0.1') }, "withdraw safe")

        console.log(await survey.getAllParticipants())
